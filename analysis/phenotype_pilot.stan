data {
  ...
  vector[N] social_mean;
  vector[N] social_contrast;
  vector[N] imitation;
  vector[N] self_value;
  vector[N] previous_choice;
  vector[N] trial_position;
...
transformed parameters {
  ...
  for (j in 1:J) {
    session_state[j, 1] = state_sd[1] * state_z[1, j];
    session_state[j, 2] = state_sd[2] * state_z[2, j];
  }
}
model {
  ...
  to_vector(state_z) ~ std_normal();
  game_intercept ~ normal(0, 0.5);
  game_social ~ normal(0, 0.5);
  regime_social ~ normal(0, 0.5);
  regime_intercept ~ normal(0, 0.5);
  order_effect ~ normal(0, 0.5);
